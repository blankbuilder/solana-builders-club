...
export function PixelCanvas({ colors, gap = 8, speed = 30, className }: PixelCanvasProps) {
  ...
  useEffect(() => {
    ...
    // Pixels are created lazily on first hover so pages with many cards
    // (e.g. a long marquee) don't pay the init cost up front.
    const resizeObserver = new ResizeObserver(() => {
      if (pixelsRef.current.length > 0) init()
    })
